...
`RemoteImageRenderer` is a `Sendable` protocol with a `@MainActor` generic method returning `any View`. It lives in the environment (`\.remoteImageRenderer`), defaulting to `AsyncImageRenderer` (SwiftUI `AsyncImage` with phase → placeholder mapping). Consumers override via `.environment(\.remoteImageRenderer, ...)`. This is why `View` has **no third-party dependency** — Kingfisher (or Nuke, etc.) is provided from outside.
