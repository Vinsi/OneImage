...
2. **Kingfisher in the sample** — `Kingfisher` (>= 8.0.0) added to the package manifest and wired into `OneImageSampleUI` only; `KingfisherRenderer` (a `RemoteImageRenderer` returning `KFImage`) demonstrated in `ContentView` "Kingfisher" section and a preview. `OneImageCore`/`OneImageView` stay dependency-free.
...
2. **`OneImageCore` ↔ `DeveloperToolsSupport` coupling** — `ImageReference.LocalSource.resource(ImageResource)` requires importing `DeveloperToolsSupport` in `OneImageCore` (`ImageReference.swift:8`), which weakens Core's platform-independence. Options: accept it, extract `ImageResource` handling, or gate the `resource` case.
...
3. Decide on the `DeveloperToolsSupport` coupling in `OneImageCore`.
