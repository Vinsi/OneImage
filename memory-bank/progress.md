...
## Working
- `OneImageCore` product: `ImageReference` with `remote`/`local` cases; `Equatable`, `Sendable`, `Hashable`.
- `OneImageView` product:
  ...
  - Modifiers: `resizable`, `renderingMode`, `interpolation`, `antialiased`, `setCornerRadius`, `foreColor`.
  - Convenience inits: `ImageView(_ ref:)`, `ImageView(_ local:)`, `ImageView(_ remote:)`, and renderer variants.
  - `ImageReference.LocalSource.asImage` public accessor.
...
- Tests: 2 OneImageCore tests + 13 OneImageView UI/render tests (Swift Testing + `ImageRenderer`) — 15 total, all passing.
...
- `OneImageCore` imports `DeveloperToolsSupport` for `ImageResource` — coupling vs. platform-independence trade-off not resolved.
